...
function Filtrar() {
    $.ajax({
        url: '/Home/ListaFiltraArchivo?N_CODIGAFOM=0&V_TIPOIGAFOM=%&V_TIPOARCH=%&V_TIPOIMAG=NOTICIAS&V_NOMBRE=%&V_DESCRIPARCH=%' + $('#txtTituloFiltrar').val() + '%&V_FECMODIFINICIO=' + $('#txtFechaInicioFiltrar').val() + '&V_FECMODIFFIN=' + $('#txtFechaFinFiltrar').val(),
        type: 'GET',
        dataType: 'json',
        data: 'data',
        beforeSend: function () {
...
function uploadFilesDoc(inputId) {
    ...
    for (var i = 0; i != files.length; i++) {
        ...
        formData.append("V_TIPOIMAG", "NOTICIAS");
